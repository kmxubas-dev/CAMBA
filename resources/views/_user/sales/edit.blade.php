<x-app-layout>
    ...
    <section class="container mx-auto flex w-full flex-col justify-center gap-4 py-5">
        ...
        <div class="mx-1 flex items-center justify-between">
            <h3 class="text-2xl">Edit Sale</h3>
        ...
        <div class="mx-auto w-full max-w-xl rounded-xl bg-white p-6 shadow-lg ring-2 ring-purple-300">
            ...
            <div class="mb-4 border-t-2 border-purple-300 pt-4">
                ...
                <div class="mt-2 flex items-center gap-3 text-gray-700">
                    <i class="ri-user-line text-xl text-pink-600"></i>
                    <p class="font-semibold">Purchased by:</p>
                    <a href="{{ route('custom.profile.show', $purchase->user->id) }}" 
                        class="font-medium text-purple-800 transition duration-150 ease-in-out hover:text-purple-900 hover:underline">
                        {{ $purchase->user->name }}
                    </a>
                </div>
            </div>
            ...
            <!-- Purchase Source -->
            <div class="mb-8 flex items-center gap-4 border-t-2 border-purple-300 pt-4">
            ...
            @if ($purchase->status === 'requested')
                <form method="POST" action="{{ route('sales.update', $purchase->id) }}" class="flex justify-center">
                    @csrf
                    @method('PUT')
                    ...
                    <button type="submit"
                            class="btn btn-pink-to-purple rounded-full px-6 py-2 font-bold shadow-lg"
                            onclick="return confirm('Are you sure you want to approve this purchase?');">
                        <i class="ri-check-double-line mr-2 text-base"></i> Approve Purchase
                    </button>
                </form>
            @endif
        </div>
